...
After several unsuccessful attempts to install Arch Linux on ZFS (there is a nice article on the [Arch Linux wiki](https://wiki.archlinux.org/index.php/Installing_Arch_Linux_on_ZFS)) I managed to finally boot it on a ZFS root filesystem.

However, since the ZFS kernel modules are not (yet) part of the standard Linux kernel, it was quite painful to have such an installation... I was experimenting in a [QEmu](http://wiki.qemu.org/Main_Page) virtual machine, and re-doing the same steps on my actual system could have been even more painful.

I also had a lot of problems mounting addintional ZFS filesystems at boot (e.g., `/home`, `/opt`, etc.)

Finally, Kernel upgrades would have been too tricky because the corresponding ZFS modules should have been recompiled in order to make the system boot. Too risky.

So I turned to another *advanced* filesystem that apparently is gaining momentum, has several features that are comparable to ZFS and is already included in the standard Kernel: [BTRFS](https://btrfs.wiki.kernel.org/index.php/Main_Page)

After several installation attempts, I finally had an Arch Linux installation with the root filesystem completely on BTRFS.

In the following sections I will describe how I did it and some tricks I implemented in order to have a very nice way of keeping track of system upgrades and make it possible to easily roll back to the previous system state in the case of an unsuccessful upgrade.
...
You can have a look at a detailed list of all its features on the [BTRFS home page](https://btrfs.wiki.kernel.org/index.php/Main_Page). What captured my interest was the following:
...
A BTRFS-formatted partition contains by default a single subvolume where users can store files. A subvolume can be thought as an independent filesystem that is attached to some parent.
...
Subvolumes appears as directories in the filesystem hierarchy. You can distinguish between them and "vanilla" directories by using the `btrfs` command that has an option to list all the child subvolumes.
...
Snapshots and subvolumes can be renamed and moved using the standard `mv` command.

They can also be deleted using the `rm -rf` command (like if they were standard directories), but a more efficient way to do it is by using the `btrfs` command.
...
BTRFS also has a ton of advanced features such as adding other devices to the filesystem, handling RAIDx setup, resizing volumes, etc. Many of them can be done on-the-fly which means that no downtime/reboot is needed.

# The setup

My goal was to have the whole SSD allocated to BTRFS, and having a setup that allowed me to take *snapshot* of the relevant part of the system.

This would have allowed me to do the following things:
...
With this setup, by snapshotting `/` I would end up with a subvolume containing everything that was needed for booting a complete system with all its functionalities.
...
You need the ISO image of the Arch Linux installation medium. You can find it [here](https://wiki.archlinux.org/index.php/Beginners'_Guide#Burn_or_write_the_latest_installation_medium). The version I used is *2012.12.01*.
...
Create a partition as big as the whole disk and save. Make sure that it is aligned if you have 4Kb-sector disks. `fdisk` should give you default values that take into account partition alignment.

I used `fdisk` to setup a plain-old MBR because my system (a Dell XPS L502x) doesn't seem to support booting from a  [GPT (UEFI standard)](http://en.wikipedia.org/wiki/GUID_Partition_Table). I tried, but I had an *"Operation system not found"* error when I rebooted. Yes, *operation* was actually mispelled in the error message :)
...
Then format the partition using BTRFS:
...
But there's a twist. The `var` directory will contain the `lib` directory where `pacman`, the Arch Linux package manager, store information about the installed packages. 

In order to have a complete *snapshot* of the system we must include this directory. So we have to make sure that the `var/lib` (and only this) *is* on the `ROOT` subvolume. We will do this by binding the `var/lib` directory on the `ROOT` subvolume to the one on the `var` subvolume.
...
The `__snapshot` and `__current` directories are used to distinguish between the subvolumes that are snapshots and those that are currently used as active subvolumes.
...
In this step we will mount the `ROOT`, in a given location so that we can install the base system on it. We will also mount `home`, `opt`, and `var` in the corresponding directories in the `ROOT` subvolume.
...
Now, the `/var/lib` on the `__current/var` subvolume will be bound to the `/var/lib` in the `__current/ROOT` subvolume and whatever is written there will end up in the right location.

### 4. Install the base system

After having chosen the mirror to be used, we will bootstrap the base system:
...
Before continuing we need to generate the `/etc/fstab` of the installed system, based on the previously created subvolumes:

{% highlight bash %}
$ genfstab -U -p /mnt/btrfs-current >> /mnt/btrfs-current/etc/fstab
{% endhighlight %}

At this point we have an initial `/etc/fstab` but we have to edit it because the bound `/var/lib` is not well recognized by `genfstab`. We should end-up with the following:
...
* `__current/ROOT` is mounted on `/`, this is specified using the `subvol` option.
* `__current/home` is mounted on `/home`
* `__current/opt` is mounted on `/opt`
* `__current/var` is mounted on `/var`
* The whole BTRFS filesystem is mounted on `/run/btrfs-root` (no `subvol` option will mount the whole filesystem). We need this because we have to bind the `/var/lib` directory to the one on the `ROOT` subvolume
...
The options specified for all the BTRFS subvolumes contain the parameters we mentioned before. We also add `nodev`, `nosuid` and `noexec` in order to further secure our installation. We do the same for the temporary filesystems. This practice is described with more details [here](https://wiki.archlinux.org/index.php/Security#Relevant_mount_options)
...
Start by chrooting to `/mnt/btrfs-current` and not to `/mnt` as written in the guide, since the filesystem where to install Arch Linux (i.e., the `__current/ROOT` subvolume) is mounted there .
...
We can take a snapshot of this initial state and keep it for reference, for example, for restoring configuration files or checking what has changed in subsequent upgrades.

In order to take a snapshot we will create a `SNAPSHOT` file in the subvolume root containing the date, time and a comment. This file will become part of the snapshot and will help the user identify what this snapshot refers to. We will remove it from `/` after the snapshot has been created:
...
The `-r` parameter will take a read-only snapshot, otherwise it will be readable as the original `__current/ROOT` subvolume.
...
Let's suppose that we started to install some packages and that at some point we did something wrong and we want to start over. We can do that by rebooting the system after changing `__current/ROOT` volume.
To do so, we will rename the `__current/ROOT` volume and take a writable snapshot of the fresh-install snapshot we took earlier.
...
When the system reboots, we will have it as if it was just installed. Of course the `/home`, `/opt` and `/var` (except `/var/lib`) might contain additional stuff because they were not part of the snapshot. But the system will be in its pristine state because all the relevant part and configurations were snapshotted.

Now, if we want, we can also remove the `__current/ROOT.old` subvolume.

### Upgrading the system

A snapshot of `__current/ROOT` could be taken just before every system upgrade. If the upgrade succeeds, then we can remove it otherwise we can roll back to the previous system state and retry the upgrade (maybe, after having changed what made the upgrade fail)

This process could be also automated with a simple script which basically does what we described in the previous section.

### Using snapshots for improving system security

Snapshots can be very useful to understand what changed in a system. For example, if we have a "trusted" snapshot like, for example, the one taken just after the system has been installed, we can compare the current state of the system against that *pristine* state and check if binaries, configuration files or libraries have changed where they shouldn't have.
...
By using a diff tool like [Meld](https://www.archlinux.org/packages/extra/any/meld/) we can compare directories and files.

In the previous pictures (click on them for a larger version), we can see that many things have changes in `/etc/`, including `groups`.
...
This diff-check can also be automated because we have all the metadata associated with the installed packages. So a script could check which packages have been installed, which files should have changed and report only the files that weren't touched by any package installation but that still have changes.
...
Usually revision control systems are more efficient in doing this kind of tracking, but if we have to manage a lot of binary data BTRFS snapshotting could be a valid alternative for managing revisions.

Think for example to video editing: a user might want to snapshot the state of her work, so he might want to put all the files related to a given project in a separate subvolume. 
